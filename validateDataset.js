 ...
 * @returns {{ yaml: object|null, body: string, error: string|null, code?: import('./dist/core').ValidationErrorCode }}
 */
function readMarkdownFile(filePath) {
  ...
  try {
    ...
    return { yaml: null, body: '', error: `Front matter error: ${message}`, code };
  }
  try {
    ...
    if (message === 'YAML front matter is not a valid object') {
      return { yaml: null, body, error: message, code: 'E_YAML_NOT_OBJECT' };
    }
    return { yaml: null, body, error: `YAML parse error: ${message}`, code: 'E_YAML_INVALID' };
  }
}
...
function validateDataset(root) {
  ...
  const errors = [];
  const pushError = (code, message, file, hint) => {
  ...
  for (const f of typeFiles) {
    ...
    if (doc.error) {
      pushError(
        doc.code || 'E_YAML_INVALID',
        `Type file error in ${rel}: ${doc.error}`,
        rel
      );
      continue;
    }
  ...
  for (const dirName of recordDirs) {
    ...
    for (const recPath of recFiles) {
      ...
      if (doc.error) {
        pushError(
          doc.code || 'E_YAML_INVALID',
          `Record file error in ${rel}: ${doc.error}`,
          rel
        );
        continue;
      }
      ...
      if (!y.id || typeof y.id !== 'string') {
        pushError('E_REQUIRED_FIELD_MISSING', `Record file ${rel} missing id or id not a string`, rel);
      }
      if (y.datasetId !== datasetId) {
        pushError(
          'E_DATASET_ID_MISMATCH',
      ...
      if (!y.createdAt || !y.updatedAt) {
        pushError(
          'E_REQUIRED_FIELD_MISSING',
          `Record file ${rel} must have createdAt and updatedAt`,
          rel
        );
      }
      if (!y.fields || typeof y.fields !== 'object') {
        pushError(
          'E_REQUIRED_FIELD_MISSING',
          `Record file ${rel} fields must be an object`,
          rel
        );
      }
    }
...
function parseArgs(args) {
  const usage = 'Usage: node validateDataset.js <datasetPath> [--json|--pretty]';
  const jsonIndex = args.indexOf('--json');
  const prettyIndex = args.indexOf('--pretty');
  if (jsonIndex !== -1 && prettyIndex !== -1) {
    return { error: makeError('E_USAGE', 'Cannot use both --json and --pretty.'), usage };
  }
  const filtered = args.filter((arg) => arg !== '--json' && arg !== '--pretty');
  if (filtered.length !== 1) {
    return { error: makeError('E_USAGE', 'Expected a dataset path.'), usage };
  }
  return {
    datasetPath: filtered[0],
    mode: jsonIndex !== -1 ? 'json' : 'pretty',
    usage
  };
}

function printResult(mode, result, exitCode) {
  const output = mode === 'json' ? formatJson(result) : formatPretty(result.errors);
  if (mode === 'json') {
...
function main() {
  ...
  const mode = parsed.mode ?? 'pretty';
  if (parsed.error) {
    const result = { ok: false, errors: [parsed.error] };
    printResult(mode, result, 2);
    if (parsed.usage && mode === 'pretty') {
      process.stderr.write(`${parsed.usage}\n`);
    }
    process.exit(2);
  }
  let rootPath = parsed.datasetPath;
  // If the argument looks like a GitHub URL, instruct the user to clone it.
  const githubRe = /^https?:\/\/github\.com\//i;
  if (githubRe.test(rootPath)) {
    const result = {
      ok: false,
      errors: [
        makeError(
          'E_USAGE',
          'Validation of remote GitHub URLs is not supported by this script. Clone the repository locally and provide its path instead.'
        )
      ]
    };
    printResult(mode, result, 2);
  ...
  try {
    if (!fs.existsSync(rootPath) || !fs.statSync(rootPath).isDirectory()) {
      const result = {
        ok: false,
        errors: [
          makeError('E_USAGE', `Dataset path ${rootPath} does not exist or is not a directory`)
        ]
      };
      printResult(mode, result, 2);
      process.exit(2);
    ...
    printResult(mode, { ok, errors: result.errors }, ok ? 0 : 1);
    process.exit(ok ? 0 : 1);
  } catch (error) {
    ...
    const result = {
      ok: false,
      errors: [
        makeError('E_INTERNAL', `Unexpected error: ${message}`)
      ]
    };
    printResult(mode, result, 2);
    process.exit(2);
