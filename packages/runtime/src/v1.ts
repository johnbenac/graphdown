...
function extractBlockCidsFromFieldsAndBody(fields: unknown, body: string): { cids: string[] } {
...
export async function openRuntimeApiV1(input: {
  ...
  for (const recordObj of parsed.recordObjects) {
    ...
    const { cids } = extractBlockCidsFromFieldsAndBody(recordObj.fields, recordObj.body);
    deepFreeze(cids);
  ...
  for (const typeObj of parsed.typeObjects) {
    const { cids } = extractBlockCidsFromFieldsAndBody(typeObj.fields, typeObj.body);
    for (const cid of cids) {
      reachableBlockCids.add(cid);
    }
  }

  const discoveredPlugins = discoverPluginObjects(input.snapshot);
