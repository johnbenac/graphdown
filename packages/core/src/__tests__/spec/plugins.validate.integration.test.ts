...
import { cidFromRawBytes, validateDatasetSnapshot } from "../../index";
...
describe("plugins validation", () => {
  ...
  it("VAL-PLUG-005: plugin bundle files must be UTF-8 decodable unless declared binary", () => {
  ...
  it("VAL-PLUG-005: binaryFiles entries allow non-UTF-8 plugin bundle files", () => {
    const manifest = manifestContent([
      "pluginId: demo",
      "gdApiVersion: 1",
      "entry: entry.js",
