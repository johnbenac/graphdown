...
function manifest(yamlLines: string[], body = ""): string {
...
describe("plugins validate", () => {
  ...
  it("VAL-PLUG-004: unsafe relative paths are rejected", () => {
    ...
    expect(
      codes.some((code) => code === "E_PLUGIN_ENTRY_INVALID" || code === "E_PLUGIN_PATH_INVALID")
    ).toBe(true);
  });
  ...
  it("VAL-PLUG-005: plugin bundle files must be UTF-8 decodable", () => {
    const snapshot = snapshotFromEntries([
      ...
      ["bad.bin", Uint8Array.of(0xff, 0xfe, 0xff)]
    ]);
