...
export function computeGdHashV1(snapshot: DatasetSnapshot, scope: HashScope): HashResult {
        ...
        if (parsedManifest.ok) {
          ...
          if (hasPluginId && hasApiVersion && !hasTypeId) {
            ...
            if (seenIds.has(identity)) {
              errors.push(
                makeError('E_DUPLICATE_ID', `Duplicate identity detected during hashing: ${identity}`, file)
              );
              continue;
            }
            ...
            pluginManifests.push({ pluginId, manifestPath: file, declaredFiles });
          }
        }
      }
      continue;
  ...
  if (scope === 'snapshot') {
    for (const manifest of pluginManifests) {
      const { pluginId, manifestPath, declaredFiles } = manifest;
      ...
      for (const relativePath of declaredFiles) {
        const identity = `plugin.${pluginId}/${relativePath}`;
        if (seenIds.has(identity)) {
          errors.push(
            makeError('E_DUPLICATE_ID', `Duplicate identity detected during hashing: ${identity}`, manifestPath)
          );
          continue;
        }
        seenIds.add(identity);
        ...
        if (!raw) {
          errors.push(
            makeError('E_PLUGIN_FILE_MISSING', `Plugin bundle file missing: ${resolvedPath}`, manifestPath)
          );
          continue;
        }
        ...
        const decoded = decodeUtf8Fatal(raw, resolvedPath, errors);
        if (decoded === null) continue;
        const normalizedText = normalizeLineEndings(decoded);
        const contentBytes = encoder.encode(normalizedText);
        const idBytes = encoder.encode(identity);
