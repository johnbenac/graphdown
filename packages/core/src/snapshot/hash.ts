  ...
  if (scope === 'snapshot') {
    ...
    for (const plugin of discovered.plugins) {
      ...
      if (Object.prototype.hasOwnProperty.call(yaml, 'binaryFiles')) {
        ...
        if (
          !Array.isArray(binaryFilesRaw) ||
          binaryFilesRaw.some((item) => typeof item !== 'string')
        ) {
          errors.push(
            makeError(
              'E_PLUGIN_KEYS_INVALID',
        ...
        for (const entry of binaryFilesRaw) {
          if (!declaredFiles.includes(entry)) {
            errors.push(
              makeError(
                'E_PLUGIN_KEYS_INVALID',
                `Plugin manifest ${manifestPath} binaryFiles entry "${entry}" must be listed in files`,
                manifestPath
              )
            );
          } else {
            binaryFiles.add(entry);
          }
        }
      }
