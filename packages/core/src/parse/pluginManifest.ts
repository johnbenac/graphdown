...
function decodeUtf8OrNull(bytes: Uint8Array): string | null {
  ...
  if (!buffer.equals(Buffer.from(decoded, 'utf8'))) {
    return null;
  }
  return decoded;
}
...
export function isPluginManifestCandidateBytes(path: string, bytes: Uint8Array): boolean {
  ...
  const recordCandidate = isRecordFileBytes(path, bytes);
  ...
  if (recordCandidate && Object.prototype.hasOwnProperty.call(yaml, 'typeId')) return false;
