...
vi.mock("@graphmd/dataset", async () => {
  const actual = await vi.importActual<typeof import("@graphmd/dataset")>("@graphmd/dataset");
  return {
    ...actual,
...
describe("readZipSnapshotFromBytes plugin parse failures", () => {
  it("throws when a plugin manifest cannot be parsed", () => {
    ...
    if (isImportError(caught)) {
      ...
      expect(caught.info.message).toMatch(/Plugin manifest parse failed/);
    }
  });
});
