...
export function planGraphdownCommit(renderResult: {
  ...
  if (duplicateKeys.length > 0) {
    const dupKey = duplicateKeys[0] as RelPath;
    const rawPaths = grouped.get(dupKey)!.map((entry) => entry.rawPath).sort();
    throw new DuplicateCommitPathError(dupKey, rawPaths);
  }
