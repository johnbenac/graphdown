...
describe("plugin non-requirements (dataset semantics must ignore plugin-defined semantics)", () => {
  it("NR-PLUG-LINK-001: no relationship or CID extraction from plugin manifest bodies or bundle contents", () => {
    ...
    const manifestBytes = mdWithFrontMatter(
      ...
      [
        ...
        "This body tries to smuggle semantics into dataset validation.",
    ...
    const recordLikePluginMdBytes = mdWithFrontMatter(
      ...
      [
        ...
        "Links/CIDs here MUST NOT be scanned by dataset validation:",
    ...
    // If dataset validation incorrectly scans plugin bodies/bundles for CID refs, this would fail with E_CID_INVALID
    // or block-reference errors. It MUST pass.
    const validation = validateDatasetSnapshot(snapshot);
    expect(validation.ok).toBe(true);

    // If dataset validation incorrectly scans plugin bodies/bundles for record links, the record-link graph would
    ...
    // If dataset validation incorrectly scans plugin bodies/bundles for CID refs during GC/export reachability,
  ...
  it("NR-PLUG-VAL-001: plugin bundle content is not executed or interpreted as additional validity rules", () => {
    // This record intentionally violates a hypothetical plugin rule; dataset validation must not care.
    const typeBytes = mdWithFrontMatter(["typeId: note", "fields: {}"]);
    const recordBytes = mdWithFrontMatter(
      ["typeId: note", "recordId: a", "fields: {}"],
      ["This is valid per dataset semantics. A plugin might want to reject it; dataset validation MUST NOT execute that logic."]
    );
    ...
    const manifestBytes = mdWithFrontMatter(
      ...
      ["Plugin declares rules/capabilities here; dataset validation must treat them as opaque (shape-checked only)."]
    );

    // If executed, this would throw. Dataset validation MUST NOT execute it during validation.
    const entryBytes = encoder.encode(
      [
        "throw new Error('If dataset validation executes plugin code during validation, this is a critical bug');",
        ""
      ].join("\n")
    );

    // A made-up plugin rules file; dataset validation MUST NOT interpret it.
  ...
  it("NR-PLUG-HASH-001: plugins do not define hashing semantics (gdhash-v1 inputs are fixed by dataset semantics)", () => {
    const typeBytes = mdWithFrontMatter(["typeId: note", "fields: {}"]);
    const recordBytes = mdWithFrontMatter(
      ["typeId: note", "recordId: a", "fields: {}"],
      ["Record body\n"]
