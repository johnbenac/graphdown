...
describe("plugin non-requirements (dataset must ignore plugin-defined semantics)", () => {
  ...
  it("NR-PLUG-VAL-001: plugin bundle content is not executed or interpreted as additional validity rules", () => {
    ...
    const recordBytes = mdWithFrontMatter(
      ["typeId: note", "recordId: a", "fields: {}"],
      ["This is valid per dataset. A plugin might want to reject it; dataset MUST NOT execute that logic."]
    );
    ...
    const manifestBytes = mdWithFrontMatter(
      ...
      ["Plugin declares rules/capabilities here; dataset must treat them as opaque (shape-checked only)."]
    );
  ...
  it("NR-PLUG-HASH-001: plugins do not define hashing semantics (gdhash-v1 inputs are fixed by dataset)", () => {
    const typeBytes = mdWithFrontMatter(["typeId: note", "fields: {}"]);
    const recordBytes = mdWithFrontMatter(
      ["typeId: note", "recordId: a", "fields: {}"],
      ["Record body\n"]
    ...
    // Dataset MUST ignore it for hashing, so the digest MUST remain unchanged.
