...
async function fetchJson<T>(input: { fetchFn: typeof fetch; url: string; signal?: AbortSignal }): Promise<T> {
...
export async function loadGitHubSnapshot(input: {
  ...
  const treeResponse = await fetchJson<GitHubTreeResponse>({
    fetchFn,
    url: `${API_BASE}/repos/${owner}/${repo}/git/trees/${resolvedRef}?recursive=1`,
    signal
  });
