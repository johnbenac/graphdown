  ...
  try {
    ...
    try {
      ...
      return {
        ok: false,
        message:
          'GitHub URL must be a repository URL like https://github.com/owner/repo or github.com/owner/repo (optionally with /tree/<ref>).',
  ...
  if (segments.length > 2) {
    ...
    if (sub === 'tree') {
      ...
      if (segments.length > 4) {
        return { ok: false, message: 'GitHub URL must point to the repository root or a /tree/<ref> branch view.' };
      }
      return {
        ok: true,
        value: {
