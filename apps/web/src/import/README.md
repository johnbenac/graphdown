...
- `readZipSnapshot.ts`
  ...
  - Delegates bytes-only parsing and filtering to `@graphdown/io-zip`, which
    applies semantic selection via `@graphdown/io`.
...
- `loadGitHubSnapshot.ts`
  ...
  - Downloads `blocks/**` and all Markdown files, then delegates semantic
    selection (records, plugin manifests, bundle requirements) to `@graphdown/io`.
  - Performs a second fetch pass for any missing plugin bundles that are present
