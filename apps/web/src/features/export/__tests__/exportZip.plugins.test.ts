...
describe("buildDatasetZipBytes plugins", () => {
  it("EXP-PLUG-001: exports plugin manifests and bundles in canonical layout", () => {
    const blobBytes = new Uint8Array(strToU8("plugin-only-block"));
  ...
  it("EXP-006: includes plugin-declared blocks even without record references", () => {
    const blobBytes = new Uint8Array(strToU8("plugin-only"));
    const cid = cidFromRawBytes(blobBytes);
    const blockPath = blockPathForCid(cid);
    ...
    const snapshot = snapshotFromEntries([
      ...
      ["plugins/demo/manifest.md", manifestText],
      ["plugins/demo/entry.js", "export {}"],
      [blockPath, blobBytes]
    ]);
