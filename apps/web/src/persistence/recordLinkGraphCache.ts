...
export function serializeRecordLinkGraphCache(
  graph: RecordLinkGraph
): PersistedRecordLinkGraphCache {
  return {
    types: [...graph.typesById.values()],
    records: [...graph.recordsByKey.values()],
    outgoingRecordLinks: [...graph.outgoingRecordLinks.entries()].map(([id, targets]) => [
      id,
      [...targets]
    ]),
    incomingRecordLinks: [...graph.incomingRecordLinks.entries()].map(([id, sources]) => [
      id,
      [...sources]
    ])
  };
...
export function deserializeRecordLinkGraphCache(
  payload: PersistedRecordLinkGraphCache
): RecordLinkGraph {
