...
export function serializeRecordLinkGraphCache(graph: RecordLinkGraph): PersistedRecordLinkGraphCache {
  return {
    ...
    outgoing: [...graph.outgoingRecordLinks.entries()].map(([id, targets]) => [id, [...targets]]),
    incoming: [...graph.incomingRecordLinks.entries()].map(([id, sources]) => [id, [...sources]])
  };
}
...
export function deserializeRecordLinkGraphCache(payload: PersistedRecordLinkGraphCache): RecordLinkGraph {
  ...
  return new RecordLinkGraphCacheImpl(
    typesById,
    recordsByKey,
    nodesByIdentity,
