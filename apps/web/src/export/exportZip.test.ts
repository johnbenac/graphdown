...
describe("exportZip", () => {
  ...
  it("EXP-HIER-001: canonical parent-based export layout nests records under their parent directory", async () => {
    const snapshot = snapshotFromEntries([
      ...
      ["records/part.md", ["---", "typeId: part", "recordId: steeringwheel", "parent: car:car-1", "fields: {}", "---"].join("\n")],
      [
        "records/spec.md",
        ["---", "typeId: spec", "recordId: steeringwheel-spec", "parent: part:steeringwheel", "fields: {}", "---"].join("\n")
      ]
    ]);
