  ...
  return {
    ...
    value: {
      ...
      listRecordKeysByType: async (typeId: string) => {
        const resolved = requireTypeId('listRecordKeysByType', typeId);
        const keys = recordKeysByTypeId.get(resolved);
        return keys ? [...keys] : [];
      },
      getType: async (typeId: string) => {
        const resolved = requireTypeId('getType', typeId);
        const view = typesById.get(resolved);
        return view ? clone(view) : null;
      },
      getRecord: async (recordKey: string) => {
        const resolved = requireRecordKey('getRecord', recordKey);
        const view = recordsByKey.get(resolved);
        return view ? clone(view) : null;
      },
      getParentRecordKey: async (recordKey: string) => {
        const resolved = requireRecordKey('getParentRecordKey', recordKey);
        const record = recordsByKey.get(resolved);
        if (!record) {
          return null;
        }
      ...
      listChildRecordKeys: async (recordKey: string) => {
        const resolved = requireRecordKey('listChildRecordKeys', recordKey);
        const kids = childrenByParentKey.get(resolved);
        return kids ? [...kids] : [];
      },
      listRootRecordKeysByType: async (typeId: string) => {
        const resolved = requireTypeId('listRootRecordKeysByType', typeId);
        const roots = rootRecordKeysByTypeId.get(resolved);
        return roots ? [...roots] : [];
      },
      getTypeCompositionComponents: async (typeId: string) => {
        const resolved = requireTypeId('getTypeCompositionComponents', typeId);
        if (!typesById.has(resolved)) {
          return null;
        }
        return clone(typeCompositionByTypeId.get(resolved) ?? []);
      },
      ...
      listRecordsByType: async (typeId: string) => {
        const resolved = requireTypeId('listRecordsByType', typeId);
        const keys = recordKeysByTypeId.get(resolved) ?? [];
        return keys
      ...
      getTypeMarkdownBytes: async (typeId: string) => {
        const resolved = requireTypeId('getTypeMarkdownBytes', typeId);
        const file = typeFileById.get(resolved);
        if (!file) {
          return null;
        }
      ...
      getRecordMarkdownBytes: async (recordKey: string) => {
        const resolved = requireRecordKey('getRecordMarkdownBytes', recordKey);
        const file = recordFileByKey.get(resolved);
        if (!file) {
          return null;
        }
        const bytes = snapshotFiles.get(file);
      ...
      listBlockCidsReferencedByRecord: async (recordKey: string) => {
        const resolved = requireRecordKey('listBlockCidsReferencedByRecord', recordKey);
        const cids = blockRefsByRecordKey.get(resolved);
        return cids ? [...cids] : [];
      },
      listReachableBlockCids: async () => [...reachableBlockCidsSorted],
      getOutgoingRecordLinks: async (recordKey: string) => {
        const resolved = requireRecordKey('getOutgoingRecordLinks', recordKey);
        return [...graph.getOutgoingRecordLinks(resolved)].sort((a, b) => a.localeCompare(b));
      },
      getIncomingRecordLinks: async (recordKey: string) => {
        const resolved = requireRecordKey('getIncomingRecordLinks', recordKey);
        return [...graph.getIncomingRecordLinks(resolved)].sort((a, b) => a.localeCompare(b));
      }
    }
  };
}
