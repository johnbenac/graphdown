...
export async function openRuntimeApiV1(input: {
  ...
  for (const recordObj of parsed.recordObjects) {
    ...
    const { cids: blockRefs } = extractBlockCidsFromRecord(recordObj.fields, recordObj.body);
    deepFreeze(blockRefs);
    blockRefsByRecordKey.set(recordKey, blockRefs);
    for (const cid of blockRefs) {
  ...
  return {
    ...
    value: {
      ...
      getBlockBytes: (cid: string) => {
        ...
        const prefix = decoded.digest[0].toString(16).padStart(2, '0');
        const path = `blocks/sha2-256/${prefix}/${cid}`;
