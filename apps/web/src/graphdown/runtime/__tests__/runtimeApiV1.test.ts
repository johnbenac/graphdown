...
test('runtime api v1 exposes record hierarchy (parent pointers only)', async () => {
  const snapshot = makeSnapshot({
    'types/note.md': typeFile('note'),
...
test('runtime api v1 composition outputs are isolated copies', async () => {
  const carType = [
    '---',
    'typeId: car',
    'fields:',
