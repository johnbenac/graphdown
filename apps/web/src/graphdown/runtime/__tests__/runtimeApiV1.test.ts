...
test('runtime api v1 listTypes + listRecordsByType return deterministic sorted views', async () => {
  ...
  assert.deepEqual(opened.value.listTypes().map((type) => type.typeId), ['note', 'task']);

  assert.deepEqual(opened.value.listRecordKeysByType('note'), ['note:a', 'note:b']);
  assert.deepEqual(opened.value.listRecordsByType('note').map((record) => record.recordKey), [
    'note:a',
    'note:b'
  ]);
});
