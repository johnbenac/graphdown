...
test('runtime api v1 open returns session for valid dataset', async () => {
  ...
  const typeView = result.value.getType('note');
  assert.ok(typeView);
  assert.equal(typeView?.typeId, 'note');
  const recordView = result.value.getRecord('note:one');
  assert.ok(recordView);
  assert.equal(recordView?.recordKey, 'note:one');
});
...
test('runtime api v1 open fails for invalid snapshot and preserves validation errors exactly', async () => {
  ...
  for (const snapshot of invalidSnapshots) {
    ...
    if (opened.ok) {
      assert.fail('Expected open errors');
    }
...
test('runtime api v1 exposes Record Link Graph adjacency', async () => {
  const snapshot = validDatasetMinimal();
  snapshot.files.set('records/two.md', new TextEncoder().encode(recordFile('note', 'two')));
  snapshot.files.set(
    'records/one.md',
    new TextEncoder().encode(recordFile('note', 'one', 'See [[note:two]].'))
  );
  const opened = await openRuntimeApiV1({ snapshot });
  assert.equal(opened.ok, true);
  if (!opened.ok) {
