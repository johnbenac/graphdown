...
test('runtime api v1 exposes record hierarchy based on parent pointers', async () => {
  ...
  assert.deepEqual(await opened.value.listRootRecordKeysByType('note'), [
    'note:linked',
    'note:root'
  ]);
});
...
test('API-SESSION-002: runtime api v1 raw bytes are returned as copies', async () => {
  const snapshot = validDatasetMinimal();
  const opened = await openRuntimeApiV1({ snapshot });
  assert.equal(opened.ok, true);
  if (!opened.ok) {
...
test('API-SESSION-002: runtime api v1 block bytes are returned as copies', async () => {
...
test('API-SESSION-002: runtime api v1 view getters return isolated copies (mutations do not affect subsequent reads)', async () => {
  const snapshot = makeSnapshot({
    'types/note.md': typeFile('note'),
    'records/one.md': [
      '---',
