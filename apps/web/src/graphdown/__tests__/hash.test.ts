...
function cid(result: ReturnType<typeof computeGdHashV1>): string {
  if (!result.ok) {
...
test('HASH-003: snapshot hash is path-independent for record files', () => {
  ...
  const digestA = cid(computeGdHashV1(snapshotA, "snapshot"));
  const digestB = cid(computeGdHashV1(snapshotB, "snapshot"));
...
test('HASH-002: schema fingerprint ignores record object changes', () => {
  ...
  const schemaDigest = cid(computeGdHashV1(base, "schema"));
  const snapshotDigest = cid(computeGdHashV1(base, "snapshot"));

  const schemaChanged = snapshot([[type[0], type[1].replace("fields: {}", "fields:\n  extra: true")], record]);
  const schemaChangedDigest = cid(computeGdHashV1(schemaChanged, "schema"));
  assert.notEqual(schemaChangedDigest, schemaDigest);

  const recordChanged = snapshot([type, recordFile("r.md", "note", "one", "Updated")]);
  const snapshotChanged = cid(computeGdHashV1(recordChanged, "snapshot"));
  assert.notEqual(snapshotChanged, snapshotDigest);
  // Schema scope ignores record body change
  assert.equal(cid(computeGdHashV1(recordChanged, "schema")), schemaDigest);
});
...
test('HASH-005: snapshot hash ignores block store bytes', () => {
  const type = typeFile("type.md", "note");
  const record = recordFile("r.md", "note", "one", "Body");
  const blockPath = "blocks/sha2-256/aa/aa" + "0".repeat(52);
  ...
  const baseDigest = cid(computeGdHashV1(base, "snapshot"));
  const changedDigest = cid(computeGdHashV1(changedBlock, "snapshot"));
...
test('HASH-001: line ending normalization yields stable hashes', () => {
  ...
  const digestUnix = cid(computeGdHashV1(snapshot([unix]), "schema"));
  const digestWindows = cid(computeGdHashV1(snapshot([windows]), "schema"));
  assert.equal(digestUnix, digestWindows);
});

test('HASH-001: non-record files are ignored', () => {
  const type = typeFile("type.md", "note");
  const base = cid(computeGdHashV1(snapshot([type]), "schema"));
  const withReadme = cid(computeGdHashV1(snapshot([type, ["README.md", "# docs\n"]]), "schema"));
