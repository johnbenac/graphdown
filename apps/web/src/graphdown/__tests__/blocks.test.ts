...
test('BLOCK-REF-001: split strings do not synthesize block references', () => {
  const cid = cidFromRawBytes(encoder.encode("rose"));
  const result = validateDatasetSnapshot(
    snapshot([
      record("types/photo.md", ["typeId: photo", "fields: {}"]),
      record(
...
test('LEGACY-001: legacy blob references are invalid', () => {
  const result = validateDatasetSnapshot(
    snapshot([
      record("types/photo.md", ["typeId: photo", "fields: {}"]),
      record(
...
test('LEGACY-002: legacy blob store paths are invalid', () => {
  const blockBytes = encoder.encode("daisy");
  const digestPrefix = decodeDaslCidString(cidFromRawBytes(blockBytes)).digest[0].toString(16).padStart(2, "0");
  const result = validateDatasetSnapshot(
    snapshot([
      record("types/photo.md", ["typeId: photo", "fields: {}"]),
      record("records/photo-1.md", ["typeId: photo", "recordId: one", "fields: {}"]),
      ["blobs/sha256/" + digestPrefix + "/" + "a".repeat(64), blockBytes]
    ])
  );
  const errors = expectErrors(result);
