...
test('BLOCK-REF-001: split strings do not synthesize block references', () => {
  const cid = cidFromRawBytes(encoder.encode("orchid"));
  const result = validateDatasetSnapshot(
    snapshot([
      record("types/photo.md", ["typeId: photo", "fields: {}"]),
      record(
...
test('VAL-LEGACY-001: legacy blob references fail validation', () => {
  const legacy = "gdblob:sha256-" + "a".repeat(64);
  const result = validateDatasetSnapshot(
    snapshot([
      record("types/photo.md", ["typeId: photo", "fields: {}"]),
      record("records/photo-1.md", ["typeId: photo", "recordId: one", "fields: {}"], `[[${legacy}]]`)
    ])
  );
  const errors = expectErrors(result);
...
test('VAL-LEGACY-002: legacy blob store paths fail validation', () => {
  const result = validateDatasetSnapshot(
    snapshot([
      record("types/photo.md", ["typeId: photo", "fields: {}"]),
