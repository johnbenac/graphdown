...
test('VAL-BLOCK-002: block bytes must match referenced CID', () => {
  const blockBytes = encoder.encode("flower");
  const cid = cidFromRawBytes(blockBytes);
...
test('CID-REF-001: split strings do not synthesize CID references', () => {
  const cid = cidFromRawBytes(encoder.encode("split"));
  const result = validateDatasetSnapshot(
    snapshot([
      record("types/photo.md", ["typeId: photo", "fields: {}"]),
      record(
...
test('LEGACY-001: legacy blob store paths are rejected', () => {
  const result = validateDatasetSnapshot(
    snapshot([
      record("types/photo.md", ["typeId: photo", "fields: {}"]),
      record("records/photo-1.md", ["typeId: photo", "recordId: one", "fields: {}"]),
      ["blobs/sha256/aa/aa" + "0".repeat(62), encoder.encode("legacy")]
    ])
  );
  const errors = expectErrors(result);
...
test('LEGACY-002: legacy blob references are rejected', () => {
  const legacyToken = "gdblob:sha256-" + "a".repeat(64);
  const result = validateDatasetSnapshot(
    snapshot([
      record("types/photo.md", ["typeId: photo", "fields: {}"]),
      record("records/photo-1.md", ["typeId: photo", "recordId: one", "fields: {}"], `[[${legacyToken}]]`)
    ])
  );
  const errors = expectErrors(result);
