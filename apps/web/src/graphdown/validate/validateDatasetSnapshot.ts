...
function validateBlockStore(
  ...
  const blockFiles = [...snapshot.files.keys()].filter((p) => p.startsWith('blocks/sha2-256/'));
  ...
  for (const path of blockFiles) {
    ...
    try {
      ...
      errors.push(makeError('E_BLOCK_PATH_INVALID', `Invalid block CID in ${path}: ${message}`, path));
      continue;
    }
    const canonicalPath = blockPathForCid(cid);
    ...
    if (digest.length !== decoded.digest.length || !digest.every((byte, index) => byte === decoded.digest[index])) {
      errors.push(
  ...
  for (const record of records) {
    ...
    for (const value of strings) {
      ...
      for (const token of legacyBlobTokens) {
        errors.push(
          makeError(
            'E_LEGACY_BLOB_REF',
            `Legacy blob reference ${token} in ${record.identity} is not supported`,
            record.file
          )
        );
      }
      for (const token of invalidCidTokens) {
        errors.push(makeError('E_CID_INVALID', `Invalid CID ${token} in ${record.identity}`, record.file));
      }
