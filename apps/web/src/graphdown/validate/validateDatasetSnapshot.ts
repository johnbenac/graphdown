...
function validateBlockStore(
  ...
  const legacyBlobFiles = [...snapshot.files.keys()].filter((p) => p.startsWith('blobs/sha256/'));
  for (const path of legacyBlobFiles) {
    errors.push(makeError('E_LEGACY_BLOB_STORE', `Legacy blob store path is not supported: ${path}`, path));
  }
  ...
  for (const path of blockFiles) {
    ...
    let decoded;
    try {
      decoded = decodeDaslCidString(filename);
    } catch {
      errors.push(makeError('E_BLOCK_PATH_INVALID', `Block filename must be a valid CID in ${path}`, path));
      continue;
    }
    if (blockPathForCid(filename) !== path) {
      errors.push(makeError('E_BLOCK_PATH_INVALID', `Block path must be canonical for CID ${filename}`, path));
      continue;
    }
    ...
    if (digest.length !== decoded.digest.length || !digest.every((byte, i) => byte === decoded.digest[i])) {
      errors.push(makeError('E_BLOCK_DIGEST_MISMATCH', `Block file ${path} digest does not match its CID`, path));
      continue;
    }
    cidToPath.set(filename, path);
  }
    ...
    for (const value of strings) {
      ...
      for (const legacy of legacyBlobTokens) {
        errors.push(
          makeError(
            'E_LEGACY_BLOB_REF',
            `Legacy blob reference ${legacy} is not supported`,
            record.file
          )
        );
      }
      for (const invalid of invalidCidTokens) {
        errors.push(makeError('E_CID_INVALID', `Invalid CID reference ${invalid}`, record.file));
      }
      for (const cid of cids) {
        const path = blockPathForCid(cid);
        if (!cidToPath.has(cid) && !snapshot.files.has(path)) {
          errors.push(
            makeError(
