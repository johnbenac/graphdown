...
2. **Validation & canonicalization**
   - `@graphdown/core` utilities validate the dataset, normalize file layout, and prune
     unused blocks.
3. **Runtime session**
   - `@graphdown/runtime` opens a session that serves as the read model for types,
     records, relationships, hierarchy, and blocks.
4. **Persistence**
...
- `@graphdown/core` - dataset parsing, validation, hashing, zip handling, and front
  matter parsing.
- `@graphdown/runtime` - session read API for types, records, links, hierarchy, and blocks.
