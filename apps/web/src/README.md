...
2. **Validation & canonicalization**
   - `graphdown/` utilities validate the dataset, normalize file layout, and
     prune unused blobs.
3. **Graph building**
...
- `components/` - reusable UI components (navigation, record editor/viewer, UI
  wrappers).
- `graphdown/` - dataset parsing, validation, graph building, hashing, zip
  handling, and front matter parsing.
