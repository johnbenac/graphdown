...
1. **Import**
   - Import screens call into `DatasetContext` actions to validate GitHub URLs or
     read zip files (including plugin manifests + bundle files).
2. **Validation & canonicalization**
   - `@graphdown/core` utilities validate the dataset, normalize file layout, and prune
     unused blocks.
3. **Runtime session**
   - `@graphdown/runtime` opens a Runtime API v1 session and becomes the read model
     for types, records, links, hierarchy, and blocks.
4. **Persistence**
   - The snapshot and UI state are serialized into storage and rehydrated on load.
5. **Editing**
...
- `@graphdown/runtime` - session-based read model for the web UI.
- `features/export/` - downloading dataset exports from the browser.
- `import/` - zip parsing + GitHub import helpers.
- `persistence/` - snapshot/UI state serialization and persistence orchestration.
