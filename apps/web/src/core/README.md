...
The `core/` directory contains the dataset parsing, validation, and graph
...
The `core/` folder contains unit tests for parsing and validation, including
`validateDatasetSnapshot.test.ts` and `markdownRecord.test.ts`.
