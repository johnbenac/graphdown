...
export function exportRepoSnapshotToZipBytes(
  ...
  for (const filePath of paths) {
    ...
    const normalized = ArrayBuffer.isView(contents)
      ? new Uint8Array(contents.buffer.slice(contents.byteOffset, contents.byteOffset + contents.byteLength))
      : new Uint8Array(contents);
    entries[filePath] = normalized;
  }
