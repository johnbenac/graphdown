...
const persistenceUnavailableError = (err?: unknown): ImportErrorState => ({
...
export function DatasetProvider({ children }: { children: React.ReactNode }) {
  ...
  const [store, setStore] = useState<ReturnType<typeof createPersistStore> | null>(null);

  useEffect(() => {
    try {
      setStore(createPersistStore({ logger: console }));
    } catch (err) {
      console.error("Persistence is required but IndexedDB is unavailable.", err);
      setActiveDataset(undefined);
      setStatus("error");
      setError(persistenceUnavailableError(err));
    }
  }, []);
  ...
  const saveActiveDataset = useCallback(
    async (
      ...
      if (!persistence) {
        throw new Error("IndexedDB is unavailable. Graphdown requires IndexedDB for persistence.");
      }
      const now = Date.now();
      const meta = {
        id: "active",
  ...
  const importDatasetZip = useCallback(
    async (file: File) => {
      if (!persistence) {
        setStatus("error");
        setError(persistenceUnavailableError());
        return;
      }
      setStatus("loading");
      setError(undefined);
      ...
      try {
        ...
        if (failedDuringPersistence) {
          console.error("Persistence is required but failed to save the dataset.", err);
          setStatus("error");
          setError(persistenceUnavailableError(err));
          return;
        }
  ...
  const commitSnapshot = useCallback(
    ...
    > => {
      if (!persistence) {
        ...
        return {
          ok: false,
          errors: [makeError("E_INTERNAL", "IndexedDB is unavailable. Graphdown requires persistence.")]
        } as const;
      }
