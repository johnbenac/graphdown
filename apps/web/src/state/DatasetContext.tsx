...
import type { DatasetMeta, LoadedDataset } from "../persistence/types";
...
const IMPORT_SAMPLE_LIMIT = 20;

function getBlobPaths(snapshot: DatasetSnapshot): string[] {
...
function buildImportReport(
  ...
  const ignoredSample = ignoredPaths.slice(0, IMPORT_SAMPLE_LIMIT);
  const rawBlobs = new Set(getBlobPaths(rawSnapshot));
  const canonicalBlobs = new Set(getBlobPaths(canonicalSnapshot));
  const dropped = [...rawBlobs].filter((path) => !canonicalBlobs.has(path));
  const droppedSample = dropped.slice(0, IMPORT_SAMPLE_LIMIT);
  return {
    ignoredCount: ignoredPaths.length,
    ignoredSample,
    droppedUnreferencedBlobsCount: dropped.length,
    droppedUnreferencedBlobsSample: droppedSample
  };
}
  ...
  const importDatasetZip = useCallback(
    async (file: File) => {
      ...
      try {
        ...
        const report = buildImportReport(ignored, rawSnapshot, canonicalSnapshot);
        const now = Date.now();
        const meta: DatasetMeta = {
          ...
          label: file.name,
          source: "import",
          ignoredFileCount: report.ignoredCount,
          ignoredFileSample: report.ignoredSample,
          droppedBlobCount: report.droppedUnreferencedBlobsCount,
          droppedBlobSample: report.droppedUnreferencedBlobsSample
        };
        await saveActiveDataset(meta, canonicalSnapshot, graphResult.graph);
        setStatus("ready");
    ...
    async (url: string) => {
      ...
      try {
        ...
        const meta: DatasetMeta = {
          ...
          graphFormatVersion: FORMAT_VERSIONS.graph,
          uiStateFormatVersion: FORMAT_VERSIONS.uiState,
          label: parsed.value.canonicalRepoUrl,
          source: "import",
          ignoredFileCount: report.ignoredCount,
          ignoredFileSample: report.ignoredSample,
          droppedBlobCount: report.droppedUnreferencedBlobsCount,
