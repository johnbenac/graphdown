...
function record(path: string, yamlLines: string[], body = ""): [string, string] {
...
describe("composition", () => {
  it("VAL-COMP-002: required component link resolves to correct type", () => {
    const result = validateDatasetSnapshot(
      snapshot([
        ...
        record("records/engine.e1/e1.md", ["typeId: engine", "recordId: e1", "fields: {}"]),
        record("records/car.c1/c1.md", ["typeId: car", "recordId: c1", "fields: {}"], "Has [[engine:e1]].")
      ])
    );
  ...
  it("VAL-COMP-002: missing required component link fails", () => {
    const result = validateDatasetSnapshot(
      snapshot([
        ...
        record("records/car.c1/c1.md", ["typeId: car", "recordId: c1", "fields: {}"], "No links here.")
      ])
    );
  ...
  it("VAL-COMP-002: link to wrong type does not satisfy requirement", () => {
    const result = validateDatasetSnapshot(
      snapshot([
        ...
        record("records/car.c1/c1.md", ["typeId: car", "recordId: c1", "fields: {}"], "Points to [[car:self]]")
      ])
    );
    expect(result.ok).toBe(false);
    if (!result.ok) {
  ...
  it("VAL-COMP-001: composition referenced types must exist", () => {
    const result = validateDatasetSnapshot(
      snapshot([
        ...
        record("records/car.c1/c1.md", ["typeId: car", "recordId: c1", "fields: {}"])
      ])
    );
    expect(result.ok).toBe(false);
    if (!result.ok) {
  ...
  it("TYPE-COMP-001: composition must be a map with only typeId + required", () => {
    const invalid = validateDatasetSnapshot(snapshot([record("types/car.md", ["typeId: car", "fields:", "  composition: []"])]));
