...
function snapshot(entries: Array<[string, string | Uint8Array]>) {
  return {
    files: new Map(entries.map(([path, content]) => [path, typeof content === "string" ? encoder.encode(content) : content]))
  };
}
...
describe("hash", () => {
  it("HASH-003: snapshot hash is path-independent for record files", () => {
    ...
    const recordContent = recordFile("records/note.one/one.md", "note", "one", "Body");
  ...
  it("HASH-004: invalid hash scope fails with E_USAGE", () => {
    const type = typeFile("type.md", "note");
    const result = computeGdHashV1(snapshot([type]), "records" as never);
