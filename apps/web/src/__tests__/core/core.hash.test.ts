...
function typeFile(path: string, typeId: string) {
  return [
    path,
...
function recordFile(path: string, typeId: string, recordId: string, body = "") {
  return [
    path,
...
function digest(result: { ok: true; digest: string } | { ok: false; errors: unknown[] }) {
