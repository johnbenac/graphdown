...
import { extractBlobRefs, extractRecordRefs } from "../../core";

describe("core wiki links", () => {
  ...
  it("BLOB-REF-001: extracts blob references", () => {
    expect(extractBlobRefs("see [[gdblob:sha256-" + "a".repeat(64) + "]]"))
      .toEqual(["a".repeat(64)]);
  });
  ...
  it("REL-001: blob references are not treated as record relationships", () => {
    expect(extractRecordRefs("see [[gdblob:sha256-" + "a".repeat(64) + "]]"))
      .toEqual([]);
  });
