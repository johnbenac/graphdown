...
describe("gaps", () => {
  ...
  it("TYPE-004 + VAL-005: fieldDefs map enforces required=true only", () => {
    ...
    const missing = record(
      "records/task.t1/t1.md",
      ["typeId: task", "recordId: t1", "fields: {}"]
    );
    const present = record(
      "records/task.t2/t2.md",
      ["typeId: task", "recordId: t2", "fields:", "  title: Hi"]
    );
  ...
  it("NR-UI-002: arbitrary keys inside fields are accepted", () => {
    ...
    const rec = record("records/note.one/one.md", [
      "typeId: note",
      "recordId: one",
      "fields:",
      "  title: Note",
