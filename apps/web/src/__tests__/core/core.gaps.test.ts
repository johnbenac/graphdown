...
describe("gaps", () => {
  ...
  it("TYPE-004: fieldDefs must be map of objects; required must be boolean when present", () => {
    ...
    const invalidRequired = record("types/flag.md", ["typeId: flag", "fields:", "  fieldDefs:", "    on:", "      required: \"yes\""]);
  ...
  it("NR-SEM-001: semantic shapes are ignored by validation", () => {
    const type = record("types/flag.md", ["typeId: flag", "fields:", "  fieldDefs:", "    enabled:", "      required: true", "      kind: boolean"]);
  ...
  it("NR-UI-002: arbitrary keys inside fields are accepted", () => {
    ...
    const rec = record("records/note-1.md", ["typeId: note", "recordId: one", "fields:", "  title: Note", "  ui:", "    widget: textarea"]);
