...
It uses the shared `@graphdown/core` domain logic plus the `@graphdown/runtime` session API as the read model, and persists the active dataset in the browser using IndexedDB (required).
...
* `@graphdown/core` shared, pure logic for dataset parsing/validation/hash/export utilities.
* `@graphdown/runtime` session read API for types, records, links, hierarchy, and block lookups.
...
1. **Import (zip or GitHub)** → loaders build a raw `DatasetSnapshot` from bytes or GitHub files, including plugin manifests and resolved bundle files.
...
4. **Open runtime session** → `openRuntimeApiV1` builds the Runtime API v1 read model for types, records, links, hierarchy, and blocks.
...
> Note: Core record discovery is content-based (LAYOUT-001), but the web app importers may choose to only load a subset of repository files for UX/performance as long as they still include every semantic file (records, types, blocks, plugin manifests, and resolved plugin bundles).
