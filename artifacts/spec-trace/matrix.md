...
## ID-002 — No reserved typeId values (testable=true)
Tests (0):
- (none)
...
## CID-001 — Canonical block digest (sha2-256) (testable=true)
Tests (1):
- apps/web/src/graphdown/__tests__/cid.test.ts — "CID-001: decodeDaslCidString reports raw codec and digest"

## CID-002 — CID format is deterministic (testable=true)
Tests (3):
- apps/web/src/graphdown/__tests__/cid.test.ts — "CID-002: cidFromRawBytes matches abc"
- apps/web/src/graphdown/__tests__/cid.test.ts — "CID-002: cidFromRawBytes matches empty bytes vector"
- apps/web/src/graphdown/__tests__/cid.test.ts — "CID-002: cidFromRawBytes matches hello"
...
## CID-REF-001 — Block references use composite wiki-link tokens (testable=true)
Tests (2):
...
Tests (2):
- apps/web/src/graphdown/__tests__/blocks.test.ts — "LEGACY-001: legacy blob store paths are rejected"
- apps/web/src/graphdown/__tests__/wikiRefs.test.ts — "LEGACY-001: flags legacy blob tokens"

## LEGACY-002 — Legacy blob store paths are invalid (testable=true)
Tests (1):
- apps/web/src/graphdown/__tests__/blocks.test.ts — "LEGACY-002: legacy blob references are rejected"
...
## VAL-BLOCK-002 — Block store files must match their path CID (testable=true)
Tests (1):
- apps/web/src/graphdown/__tests__/blocks.test.ts — "VAL-BLOCK-002: block bytes must match referenced CID"

## GC-001 — Reachable block set is computed from CID references (testable=true)
Tests (1):
