...
Graphdown is a web application for Markdown-first datasets defined by the Graphdown standard. It ships the spec, the web app for importing/browsing/editing datasets, and core libraries for working with dataset snapshots.
...
- Import datasets from GitHub repo roots or `/tree/<ref>` URLs (types/, records/, and blobs/sha256 paths are fetched) or from uploaded zip archives. File/issue/subdirectory URLs are rejected.
- Runs the core validator, builds a link graph, and persists the loaded snapshot offline (IndexedDB with in-memory fallback).
...
- Export a dataset zip in canonical layout (types/records + referenced blobs).
...
- Snapshots can be loaded from zip bytes (`loadRepoSnapshotFromZipBytes`).
...
- Export helpers generate dataset zips from canonical snapshots (`exportDatasetZipBytes`).
