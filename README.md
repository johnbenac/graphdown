...
Everything else in the repository is **non-semantic** and ignored by the dataset SDK (BLOCK-LAYOUT-003).
...
Optional keys:
- `meta` (object; opaque to the dataset SDK)
- `config` (object; opaque to the dataset SDK)
- `requires` (array of strings; opaque to the dataset SDK)
- `blocks` (array of block CID strings; interpreted by the dataset SDK for reachability + validation)
...
### Plugin non-requirements (dataset SDK must stay “dumb”)

The dataset SDK MUST NOT:
...
(Body is opaque to the dataset SDK; it is not scanned for links/CIDs.)
```
...
* `packages/core/src/` — legacy compatibility shim re-exporting dataset behavior
