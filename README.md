...
- Requires `types/` and `records/`; record files must live under `records/<recordTypeId>.<recordId>/`.
...
- Export the dataset (`types/` + `records/` + referenced blobs) as a zip.
...
- Snapshots can be loaded from zip bytes (`loadDatasetSnapshotFromZipBytes`).
