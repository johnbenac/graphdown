...
- Builds a link graph and persists the loaded snapshot offline (IndexedDB with in-memory fallback).
...
- Export a canonical dataset zip (types/records + referenced blobs).

## Dataset layout
```
dataset-root/
├── types/
│   ├── <typeId>.md            # type records
└── records/
    ├── <typeId>.<recordId>/
    │   └── <recordId>.md
└── blobs/
