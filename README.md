...
- a **dataset domain library** (`packages/dataset/src`) for parsing, validation, hashing, canonicalization, and import/export helpers (the core package is now a shim)
...
Everything else in the repository is **non-semantic** and ignored by dataset validation (BLOCK-LAYOUT-003).
...
### Plugin non-requirements (dataset semantics must stay “dumb”)

Dataset validation MUST NOT:
