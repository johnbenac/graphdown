...
## Validation rules
- Requires `types/` and `records/`; record files must live under a valid `records/<typeId>.<recordId>/` hierarchy.
...
- Composition: types may declare `fields.composition.<name> = { typeId, required }`. Records must satisfy required components via wiki-links (`[[id]]`) found in front matter and Markdown bodies. Unknown component types and constraint violations are reported.
...
- Runs the dataset validator, builds a link graph, and persists the loaded snapshot offline (IndexedDB with in-memory fallback).
...
- Export a dataset zip (canonical layout + referenced blobs only).

## Dataset layout
```
dataset-root/
├── types/
│   ├── <typeId>.md            # type records (id starts with type:, typeId: sys:type)
└── records/
    └── <typeId>.<recordId>/
        └── <recordId>.md
```
...
- Export helpers generate dataset zips (`exportDatasetZipBytes`).
