...
<!-- req:id=GOV-003 title="Spec trace must include package tests" testable=true -->
### GOV-003 — Spec trace must include package tests

The spec-trace scanner MUST discover requirement-labeled tests across all package `src` directories so that traceability does not regress when tests move between packages.
