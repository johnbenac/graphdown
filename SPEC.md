...
A **block CID** is a DASL CIDv1 string with the following constraints:

* string form MUST start with `b`
* base32 MUST be RFC4648 lowercase with no padding
* decoded bytes MUST be exactly 36 bytes:
  * version: `0x01`
...
<!-- req:id=BLOCK-002 title="Block CID format is deterministic" testable=true -->
### BLOCK-002 — Block CID format is deterministic

A block CID MUST be constructed from the sha2-256 digest:

* CID bytes: `[0x01, 0x55 or 0x71, 0x12, 0x20, ...digest32]`
* CID string: base32 RFC4648 lowercase with no padding, prefixed by `b`
...
<!-- req:id=BLOCK-REF-001 title="Block references use wiki-link tokens" testable=true -->
### BLOCK-REF-001 — Block references use wiki-link tokens
...
* unwrap `[[...]]`
* trim surrounding whitespace
* require the inner text to match a CID string exactly
...
Core MUST treat legacy blob references of the form `[[gdblob:sha256-<hex>]]` as invalid.

Validation MUST fail if any record content contains legacy blob reference tokens.

<!-- req:id=LEGACY-002 title="Legacy blob stores are invalid" testable=true -->
### LEGACY-002 — Legacy blob stores are invalid

Core MUST reject any snapshot that contains files under `blobs/sha256/`.

Validation MUST fail if any legacy blob store paths are present.
...
2. The block file’s computed digest (BLOCK-001) MUST equal the digest encoded in the referenced CID.
