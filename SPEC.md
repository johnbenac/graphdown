...
A **DASL CID** is the CIDv1 string that identifies a block by its sha2-256 digest (CID-001).

### Block reference

A **block reference** is a wiki-link token whose inner text is a DASL CID:
...
<!-- req:id=ID-002 title="No reserved typeId values" testable=true -->
### ID-002 — No reserved typeId values

Graphdown reserves no `typeId` values. Implementations MUST NOT reject identifiers solely because they match a reserved word.
...
Changing the bytes of a referenced block necessarily changes its digest and therefore requires changing the referencing record content; therefore the dataset snapshot fingerprint changes.

## 3.2 Block identity

<!-- req:id=CID-001 title="Canonical block digest (sha2-256)" testable=true -->
### CID-001 — Canonical block digest (sha2-256)

A block digest MUST be computed as:

* `SHA-256` over the block’s raw bytes (no normalization, no decoding).

The digest is embedded into the DASL CID.

<!-- req:id=CID-002 title="CID format is deterministic" testable=true -->
### CID-002 — CID format is deterministic

A DASL CID MUST have the exact form:

`b<base32>`

Where `<base32>` matches `^[a-z2-7]{58}$` and decodes to 36 bytes with:

* version: `0x01`
* codec: `0x55` (raw) or `0x71` (drisl)
* hash type: `0x12` (sha2-256)
* hash size: `0x20` (32 bytes)
* digest: 32 bytes
...
<!-- req:id=CID-REF-001 title="Block references use composite wiki-link tokens" testable=true -->
### CID-REF-001 — Block references use composite wiki-link tokens

A block reference is expressed only as a wiki-link token:

`[[bafk...]]`

Where the inner text is a DASL CID.
...
* unwrap `[[...]]`
* trim surrounding whitespace
* require the inner text to match the DASL CID format (CID-002) exactly

Tokens that do not match this shape MUST be ignored for block reference extraction.

---

<!-- req:id=LEGACY-001 title="Legacy blob references are invalid" testable=true -->
### LEGACY-001 — Legacy blob references are invalid

Legacy blob references of the form `[[gdblob:sha256-<digest>]]` MUST cause validation failure.

<!-- req:id=LEGACY-002 title="Legacy blob store paths are invalid" testable=true -->
### LEGACY-002 — Legacy blob store paths are invalid

Any file located under `blobs/sha256/` MUST cause validation failure.
...
2. The block file’s computed digest (CID-001) MUST equal the referenced CID digest exactly.

Validation MUST fail otherwise.

<!-- req:id=VAL-BLOCK-002 title="Block store files must match their path CID" testable=true -->
### VAL-BLOCK-002 — Block store files must match their path CID

For every file under `blocks/sha2-256/<p>/<cid>`:

The computed digest of the file bytes (CID-001) MUST equal the digest encoded in `<cid>`.

Validation MUST fail otherwise.

## 9.1 Block garbage collection

<!-- req:id=GC-001 title="Reachable block set is computed from CID references" testable=true -->
### GC-001 — Reachable block set is computed from CID references

The reachable block set is defined as the set of CIDs referenced by block references extracted from all record objects per CID-REF-001/CID-REF-002.
