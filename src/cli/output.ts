...
const hintMap: Partial<Record<ValidationErrorCode, string>> = {
  ...
  E_GITHUB_URL_INVALID_FORMAT:
    'Supported: github.com/<owner>/<repo> or github.com/<owner>/<repo>/tree/<ref>.',
  E_GITHUB_NOT_FOUND:
    'Check repo name; if private, provide GITHUB_TOKEN.',
  E_GITHUB_AUTH_REQUIRED:
    'Set GITHUB_TOKEN env var (or make repo public).',
  E_GITHUB_RATE_LIMITED:
    'Set GITHUB_TOKEN or wait for the rate limit to reset.',
  E_GITHUB_REF_NOT_FOUND:
    'Use --ref <ref> or a /tree/<ref> URL.',
  E_GITHUB_FETCH_FAILED:
    'Re-run; if persistent open an issue.',
