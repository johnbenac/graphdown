...
- Legacy blob references `[[gdblob:sha256-...]]` are explicitly excluded from record relationships.
...
**Definition:** A record references a block if it contains a block wiki-link token:
`[[<cid>]]`
...
- block bytes must hash to the referenced digest (VAL-BLOCK-001/002)
...
- reachable block set (block references)
