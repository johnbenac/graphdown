...
Graphdown works with datasets as files. Internally, the dataset SDK uses a snapshot abstraction.
...
The dataset SDK does not treat directory names as semantic.
...
The Graphdown dataset SDK can operate on any snapshot and discovers record files by content (LAYOUT-001).
