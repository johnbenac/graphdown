...
Record/type → block edges implied by block references extracted from record/type body and fields, plus plugin manifest `blocks[]`.

- Nodes: records + types + block CIDs
- Edges: `recordKey -> cid`, `typeId -> cid`, and `pluginId -> cid` (from `manifest.blocks[]`)
