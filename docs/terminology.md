...
The dataset SDK treats `recordKey` as computed and MUST NOT store it as a separate YAML field.
...
Record/type→block edges implied by block references extracted from record/type body/fields, plus
explicit plugin manifest `blocks[]` dependencies.

- Nodes: records, types, plugin manifests + block CIDs
- Edges: `recordKey -> cid`, `typeId -> cid`, `pluginId -> cid`
