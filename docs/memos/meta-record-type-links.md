...
- Graphdown graph builder (`apps/web/src/graphdown/graph/graph.ts`) extracts links from record bodies/YAML; `GraphTypeDef` only stores `recordTypeId`, `typeRecordId`, `fields`; there is no type-level linking.
- Validator (`apps/web/src/graphdown/validate/validateDatasetSnapshot.ts`) enforces per-record invariants (id uniqueness, typeId alignment, directory layout) and schema optionality; it does **not** validate type-to-type constraints.
...
3) **Hard validation (composition as integrity rule)**  
   ...
   - Validator checks presence of required links; imports/exports would fail on missing associations.  
...
- **Validator**: `apps/web/src/graphdown/validate/validateDatasetSnapshot.ts` to surface warnings/errors when records of a type lack mandated links; new error codes if enforced.
- **Graph model**: `apps/web/src/graphdown/graph/graph.ts` `GraphTypeDef` may need to carry dependency metadata; link extraction stays record-based unless composition auto-creates links.
...
- **Tests/fixtures**: update graphdown/web tests and sample datasets to include type-level dependency metadata and example records satisfying them.
