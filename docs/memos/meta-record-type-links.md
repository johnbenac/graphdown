...
3) **Hard validation (composition as integrity rule)**  
  ...
  - Validator checks presence of required links; import/export flows would fail on missing associations.  
...
- **Import UI**: `apps/web/src/import/github/mapGitHubError.ts` or `apps/web/src/state/DatasetContext.tsx` for any new warning/error codes surfaced in the web app.
