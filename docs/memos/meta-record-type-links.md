...
- Core graph builder (`apps/web/src/graphdown/graph/graph.ts`) extracts links from record bodies/YAML; `GraphTypeDef` only stores `recordTypeId`, `typeRecordId`, `fields`; there is no type-level linking.
